...
std::string vocab_file, file_head;
...
void insert(CRECID *pq, CRECID new_, int size) {
    int j = size - 1, p;
    pq[j] = new_;
    while ( (p=(j-1)/2) >= 0 ) {
...
/* delete_ entry from priority queue */
void delete_(CRECID *pq, int size) {
    int j, p = 0;
    pq[p] = pq[size - 1];
...
int merge_write(CRECID new_, CRECID *old, FILE *fout) {
    if (new_.word1 == old->word1 && new_.word2 == old->word2) {
        old->val += new_.val;
...
int merge_files(int num, std::string cooccur_file) {
    int i, size;
    long long counter = 0;
    CRECID *pq, new_, old;
    char filename[200];
    ...
    fout = fopen(cooccur_file.c_str(),"w");;
    ...
    for (i = 0; i < num; i++) {
        sprintf(filename,"%s_%04d.bin",file_head.c_str(),i);
        fid[i] = fopen(filename,"rb");
        if (fid[i] == NULL) {fprintf(stderr, "Unable to open file %s.\n",filename); return 1;}
        auto ret_val = fread(&new_, sizeof(CREC), 1, fid[i]);
        (void) ret_val;
        new_.id = i;
        insert(pq,new_,i+1);
    }
    ...
    while (size > 0) {
        ...
        i = pq[0].id;
        delete_(pq, size);
        ret_val = fread(&new_, sizeof(CREC), 1, fid[i]);
        (void) ret_val;
        if (feof(fid[i])) size--;
        else {
            new_.id = i;
...
int get_cooccurrence(const std::string corpus_file, const std::string cooccur_file) {
    ...
    if (verbose > 1) fprintf(stderr, "Reading vocab from file \"%s\"...", vocab_file.c_str());
    fid = fopen(vocab_file.c_str(),"r");
    if (fid == NULL) {fprintf(stderr,"Unable to open vocab file %s.\n",vocab_file.c_str()); return 1;}
    ...
    qsort(cr, ind, sizeof(CREC), compare_crec);
    write_chunk(cr,ind,foverflow);
    sprintf(filename,"%s_0000.bin",file_head.c_str());
    fclose(fid);
    ...
    free(bigram_table);
    free(vocab_hash);
    return merge_files(fidcounter + 1, cooccur_file); // Merge the sorted temporary files
}
...
int cooccur(const std::string corpus_file, const std::string cooccur_file,
            const int verbose_ = 2, const int symmetric_ = 1,
            const int window_size_ = 15,
            const std::string vocab_file_ = "vocab.txt",
            const float memory_ = 3, const long long max_product_ = 0,
            const long long overflow_length_ = 0,
            const std::string overflow_file_ = "overflow") {
  int i;
  real rlimit, n = 1e5;
  verbose = verbose_;
  ...
  file_head = overflow_file_;
  memory_limit = memory_;

  /* The memory_limit determines a limit on the number of elements in
   * bigram_table and the overflow buffer */
  /* Estimate the maximum value that max_product can take so that this limit is
   * still satisfied */
  ...
  if(max_product_ > 0){
    max_product = max_product_;
  }

  if(overflow_length_ > 0){
    overflow_length = overflow_length_;
  }

  return get_cooccurrence(corpus_file, cooccur_file);
}

} // end namespace cooccur